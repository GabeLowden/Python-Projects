...
#RQ2
    def go_to(self, direction):
        ...
        """
        "*** YOUR CODE HERE ***"
        exits = self.place.exits
        if direction in exits:
            self.place = exits[direction][0]
...
#RQ3
    def talk_to(self, person):
        ...
        if person in self.place.characters:
            characters = self.place.characters[person]
            print(f"{characters.name} says: {characters.talk()}")
        else:
...
#RQ4
    def take(self, thing):
        ...
        if thing in self.place.things:
            ...
            del self.place.things[thing]  # Remove the item from the place
